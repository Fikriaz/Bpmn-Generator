...
export default function ViewDetailPage() {
  ...
  /* ---------- Utilities for test data editing ---------- */
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (list.length) {
          if (scenarioId) {
            selectedScenario = list.find(
              (s) => s.path_id === scenarioId || s.path_id === `P${scenarioId}`
            );
          }
          if (!selectedScenario) selectedScenario = list[pathIndex] || list[0];
        }
        ...
        if (!cancelled) setError(err.message || "Failed to fetch scenario");
      } finally {
  ...
  // re-highlight if scenario changes
  useEffect(() => {
  ...
  const getStatusDisplay = (sc) => {
    if (!sc) return "-";
    const message = sc?.expected_result?.message?.trim();
    return message || "-";
  };

  /* ---------- BPMN viewer controls ---------- */
  const handleZoomFit = () => {
    if (viewerRef.current) viewerRef.current.get("canvas").zoom("fit-viewport");
  };
  const handleZoomIn = () => {
  ...
  const handleSaveDescription = async () => {
    setSaving(true);
    try {
      const updatedInputData = buildInputDataPayload(testDataList);
      await saveToBackend({
  ...
  const handleSaveExpectedResult = async () => {
    ...
    try {
      ...
      setScenario((p) => ({
        ...p,
        expected_result: { ...p?.expected_result, message: tempExpectedResult }
      }));
  ...
  const handleEditActionSteps = () => {
    ...
    setTempActionSteps(currentSteps.length ? currentSteps : [""]);
  };
  const handleSaveActionSteps = async () => {
    ...
    try {
      ...
      setScenario((p) => ({ ...p, scenario_step: formattedSteps }));
            ...
            <section className="space-y-4">
              ...
              {!editingActionSteps ? (
                ...
                <div className="space-y-4">
                    ...
                    {tempActionSteps.map((step, index) => (
                      <div key={index} className="flex items-center space-x-3">
                        ...
                        <input
                          type="text"
                          value={step}
                          onChange={(e) => setTempActionSteps((arr) => arr.map((s, i) => (i === index ? e.target.value : s)))}
                        ...
                        />
                        <button onClick={() => setTempActionSteps((arr) => arr.filter((_, i) => i !== index))} className="p-2 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors">
                          <X className="w-4 h-4" />
                        </button>
                      </div>
                    ))}
                  ...
                  <button onClick={() => setTempActionSteps((arr) => [...arr, ""])} className="flex items-center space-x-2 text-[#2185D5] hover:text-[#1D5D9B] hover:bg-blue-50 px-3 py-2 rounded-lg transition-colors text-sm">
                  ...
                  {testDataList.length > 0 ? (
                    <div className="space-y-4">
                      {testDataList.map((item, index) => (
                        <TestDataItemView key={item.id} item={item} index={index} />
                      ))}
                    </div>
                  ) : (
...
/* ---------- Small components for Test Data ---------- */
function TestDataItemView({ item, index }) {
  if (item.type === "array") {
    return (
...
function TestDataItemEdit(props) {
  const {
    item,
    index,
    updatePrimitive,
    ...
    return (
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <button onClick={() => removeField(item.id)} className="px-2 py-1 text-red-600 hover:bg-red-50 rounded" title="Remove field">
            Hapus
          </button>
        </div>
      </div>
